...
function mapMissingToHealthCategory(item: MissingDocumentItem): HealthMapCategoryId {
  const type = item.documentType.toLowerCase();

  if (type.includes("bank") || type.includes("invoice") || type.includes("compliance")) {
    return "financial";
  }
...
function getCategoryLabelForMissing(item: MissingDocumentItem) {
  return (
    healthCategoryMeta.find((category) => category.id === mapMissingToHealthCategory(item))
      ?.title ?? "Company Compliance"
  );
}

function getPriorityBadge(kind: PriorityKind, item: ComplianceDocumentRecord | MissingDocumentItem): PriorityBadge {
  if (kind === "expired") {
    return {
      label: "critical",
  ...
  const daysRemaining = daysFromSnapshot((item as ComplianceDocumentRecord).expiryDate);
...
function getPriorityAccent(kind: PriorityKind, item: ComplianceDocumentRecord | MissingDocumentItem) {
  if (kind === "expired") {
    return "bg-rose-600";
  }

  if (kind === "missing") {
    const missingItem = item as MissingDocumentItem;
    return missingItem.documentType.toLowerCase().includes("compliance")
      ? "bg-brand-700"
      : "bg-amber-500";
  }
...
function getPriorityActionLabel(kind: PriorityKind, item: ComplianceDocumentRecord | MissingDocumentItem) {
  if (kind === "expired") {
  ...
  return daysFromSnapshot((item as ComplianceDocumentRecord).expiryDate) <= 14 ? "View" : "Open record";
}
...
function buildHealthMap(data: ComplianceCentreData) {
  ...
  return healthCategoryMeta.map((category) => {
    ...
    const missingCount = data.missingRequiredDocuments.filter(
      (item) => mapMissingToHealthCategory(item) === category.id,
    ).length;
...
function PrioritySection({
  ...
  items: Array<ComplianceDocumentRecord | MissingDocumentItem>;
  kind: PriorityKind;
  onAction: (kind: PriorityKind, item: ComplianceDocumentRecord | MissingDocumentItem) => void;
}) {
  return (
      ...
      {items.length > 0 ? (
        <div className="space-y-4">
          {items.map((item) => {
            ...
            const category =
              "name" in item ? getCategoryLabelForDocument(item) : getCategoryLabelForMissing(item);
            const accentClass = getPriorityAccent(kind, item);
            const titleText = "name" in item ? item.name : item.documentType;
            const statusText =
              kind === "missing"
                ? "Required document"
                : formatExpiryMessage((item as ComplianceDocumentRecord).expiryDate);
...
export function ClientComplianceCentrePage() {
    ...
    () => [
      ...
      },
      {
        ...
        items: [...data.expiringDocuments].sort(
          (left, right) => daysFromSnapshot(left.expiryDate) - daysFromSnapshot(right.expiryDate),
        ),
  ...
  function handlePriorityAction(kind: PriorityKind, item: ComplianceDocumentRecord | MissingDocumentItem) {
    const itemLabel = "name" in item ? item.name : item.documentType;
    ...
    showNotice(
      "info",
      "Renewal window open",
      `${itemLabel} is inside its renewal window. Review it before ${formatDateLabel((item as ComplianceDocumentRecord).expiryDate)}.`,
    );
  }
