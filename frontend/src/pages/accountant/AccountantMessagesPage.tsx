...
export function AccountantMessagesPage() {
  ...
  const targets = useMemo<MessageTarget[]>(
    () => [
      ...portal.clientWorkflow.documents.map((document) => ({
        ...
        label: `${document.clientName} / ${document.documentType}`,
      })),
      ...portal.clientWorkflow.requests.map((request) => ({
        id: request.id,
        kind: "request" as const,
        label: `${request.clientName} / ${request.title}`,
      })),
    ],
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        description="Accountants comment in context. Document threads stay with the file, and request threads stay with the follow-up task."
      ...
      <section className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]">
        <SurfaceCard className="space-y-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Pick the exact document or request thread that needs attention.
            </p>
          </div>
            ...
            {targets.map((target) => (
              ...
              >
                ...
                <p className={`mt-1 text-sm ${target.id === selectedTargetId ? "text-white/75" : "text-slate-500"}`}>
                  {target.kind === "document" ? "Document thread" : "Request thread"}
                </p>
              </button>
            ))}
          </div>
        ...
        <SurfaceCard className="space-y-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              This thread is part of the audit-ready workflow for the selected record.
            </p>
          </div>
          <CommentThread
