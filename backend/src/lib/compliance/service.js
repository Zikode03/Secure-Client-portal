// Friendly guide: this module (service) supports the Secure Client Portal workflow.
...
export async function getClientActionCenter(reqUser, clientId) {
  ...
  const acts = obligations.filter((o) => ["overdue", "due_soon"].includes(normStatus(o.status))).map((o) => ({ type: "obligation", source: o.source, title: `${o.obligationType} ${normStatus(o.status) === "overdue" ? "overdue" : "due soon"}`, urgency: normStatus(o.status) === "overdue" ? 1000 : 500, dueDate: o.dueDate, directLink: `/client/documents?obligationId=${encodeURIComponent(o.id)}&obligation=${encodeURIComponent(o.obligationType)}&source=${encodeURIComponent(o.source)}`, obligationId: o.id }))
    .concat(alerts.map((a) => ({ type: "alert", source: a.source, title: a.title, urgency: sev(a.severity) * 200 + (a.status === "escalated" ? 150 : 0), directLink: `/client/notifications?alertId=${encodeURIComponent(a.id)}`, alertId: a.id })))
